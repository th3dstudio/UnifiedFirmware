...
ENVI_NAME = "melzi_optiboot_optimized"
        ...
        for profile in config['compile-profile']:
            ...
            if saveFile(MARLIN_PATH+configFile,tmpConfigFileData):
                ...
                try:
                    ...
                    pass
                #copy binary file
                print("Copy binary output file to "+AUTO_COMPILE_DIR + profile['profile-name'] + "/" + "firmware.hex")
                try:
                    shutil.copy(".pio/build/"+ ENVI_NAME +"/firmware.hex",AUTO_COMPILE_DIR + profile['profile-name'] + "/" + "firmware.hex")
                    taskList[taskCount]["status"] = True
                except:
                    print("Cannot save firmware.hex")
