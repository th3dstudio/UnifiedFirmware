...
ENVI_NAME = "mks_robin_nano35_kp3s"
        ...
        for profile in config['compile-profile']:
            ...
            if saveFile(MARLIN_PATH+configFile,tmpConfigFileData):
                ...
                try:
                    ...
                    pass
                #copy binary file
                print("Copy binary output file to "+AUTO_COMPILE_DIR + profile['profile-name'] + "/" + "Robin_nano.bin")
                try:
                    shutil.copy(".pio/build/"+ ENVI_NAME +"/Robin_nano.bin",AUTO_COMPILE_DIR + profile['profile-name'] + "/" + "Robin_nano.bin")
                    taskList[taskCount]["status"] = True
                except:
                    print("Cannot save Robin_nano.bin")
